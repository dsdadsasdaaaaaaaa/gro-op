...
export function createAdvisor(ctx) {
  let root = null, refs = {}, brief = null, messages = [], sending = false;

  function fieldLabel(f) { return ({ temp_min_c: 'temp min', temp_max_c: 'temp max', humidity_min: 'RH min', humidity_max: 'RH max', vpd_min: 'VPD min', vpd_max: 'VPD max' })[f] || f; }
  ...
  function renderBrief() {
    ...
    replaceChildren(refs.brief,
      ...
      b.photo_requests?.length || b.tasks?.length ? h('div', { class: 'sec row' }, b.photo_requests?.length ? h('a', { class: 'pill', href: '#/journal?filter=request' }, icon('camera'), `${b.photo_requests.length} photo request${b.photo_requests.length > 1 ? 's' : ''}`) : null, b.tasks?.length ? h('a', { class: 'pill', href: '#/journal?filter=task' }, icon('list'), `${b.tasks.length} task${b.tasks.length > 1 ? 's' : ''}`) : null) : null);
  }
  ...
  async function runBrief() {
    ...
    refs.runBtn.disabled = false; replaceChildren(refs.runBtn, icon('sparkles'), 'Run brief now');
  }

  function renderMessages() {
    replaceChildren(refs.msgs, messages.length ? messages.map((m) => h('div', { class: `msg ${m.role}` }, m.content, h('time', { datetime: m.created_at }, fmtDateTime(m.created_at)))) : h('div', { class: 'empty' }, 'Ask anything about the grow — feeding, training, what a leaf symptom means.'));
  ...
  async function send() {
    ...
    sending = true; refs.input.value = ''; refs.sendBtn.disabled = true;
  ...
  async function clearChat() {
    if (!(await confirmDialog({ title: 'Clear the conversation?', message: 'The advisor forgets this chat. Your logs and photos stay.', confirmText: 'Clear', danger: true }))) return;
  ...
  function mount(el) {
    ...
    refs.runBtn = h('button', { class: 'btn ghost', onclick: runBrief }, icon('sparkles'), 'Run brief now');
    ...
    replaceChildren(root,
      h('header', { class: 'page-head' }, h('div', null, h('h1', null, 'Advisor'), h('div', { class: 'date' }, adv && adv.advisor_enabled === false ? 'The advisor is off — add an Anthropic API key to the add-on to turn it on.' : 'Claude watches the tent, the camera and your journal.')), refs.runBtn),
